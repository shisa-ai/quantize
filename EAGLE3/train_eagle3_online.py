...
def parse_args():
    ...
    parser.add_argument("--flex-block-size", type=int, default=128, help="Block size for flex attention (when enabled).")
...
def main():
    ...
    if args.attention_backend == "flex_attention":
        flex_block = max(16, args.flex_block_size)
        eagle_cfg = getattr(draft_model.config, "eagle_config", {}) or {}
        eagle_cfg["flex_block_size"] = flex_block
        setattr(draft_model.config, "eagle_config", eagle_cfg)
        setattr(draft_model.config, "flex_block_size", flex_block)
        print_with_rank(f"Using flex attention block size {flex_block}")
